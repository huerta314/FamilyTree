...
int QueryOperations::Add(Query query){
    ...
    if(query.ruleOrFactIdentifier.compare("Fact")) kb.AddFact(query);
    else    rb.AddRule(query);
}

int QueryOperations::Remove(Query query){
    if(query.ruleOrFactIdentifier.compare("Fact")) kb.RemoveFact(query);
    else rb.RemoveFact(query);
}
